...
namespace NiTiErp.Application.Dapper.Implementation
{
    public class TonGiaoService : ITonGiaoService
    {
        ...
        public async Task<List<TonGiaoViewModel>> TonGiaoGetList(string bangId, string id2, string id3, string parameters)
        {
            using (var sqlConnection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                try
                {
                    var query = await sqlConnection.QueryAsync<TonGiaoViewModel>(

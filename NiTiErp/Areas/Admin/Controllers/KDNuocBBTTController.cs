...
{
    public class KDNuocBBTTController : BaseController
    {
        ...
        public KDNuocBBTTController(IHostingEnvironment hostingEnvironment,
        ...
        public IActionResult Index()
        {
            ...
            var result = _authorizationService.AuthorizeAsync(User, "KiemDinhNuocKeQua", Operations.Read);
        ...
        [HttpGet]
        public IActionResult ListGDNNuocTTXL(string corporationId, string phongdanhmucId, string keyword, int page, int pageSize)
        {
            ...
            var model = _giaydenghiDMCungCapNuocService.Get_GiayDeNghiDMCungCapNuoc_AllByTTXLKD(khuvuc, phong, tukhoa, page, pageSize);
        ...
        [HttpPost]
        public IActionResult XLKDNuocBBTT(GDNXuLyKiemDinhNuocViewModel giaydenghi)
        {
            ...
            {
                var result = _authorizationService.AuthorizeAsync(User, "KiemDinhNuocKeQua", Operations.Update);
                if (result.Result.Succeeded == false)
                {
                ...
                var model = _gdnxulykiemdinhnuocService.UpdateGDNXuLyKiemDinhNuocBBTT(giaydenghi, createDate, createBy);
                return new OkObjectResult(model);
            }
        }

        [HttpPost]
        public IActionResult XLKDNuocBBTTUp(GDNXuLyKiemDinhNuocViewModel giaydenghi)
        {
            if (!ModelState.IsValid)
            {
                IEnumerable<ModelError> allErrors = ModelState.Values.SelectMany(v => v.Errors);
            ...
            else
            {
                ...
                var model = _gdnxulykiemdinhnuocService.UpdateGDNXuLyKiemDinhNuocBBTTUp(giaydenghi, createDate, createBy);
                return new OkObjectResult(model);
            }
        }

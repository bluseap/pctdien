var addeditbbttdController = function () {
    ...
    function registerEvents() {
        ...
        $('#btnSaveBBTTDieuChinhSanLuong').on('click', function () {
            ...
            if (isgiaydenghi == 1) {
                saveGDNXuLyKDDienBBTT();
            } else if (isgiaydenghi == 2) {
                updateGDNXuLyKDDienBBTT();
            }
            else {
    ...
    function loadTableGiayDeNghiDMCungCapDien(isPageChanged) {
        var template = $('#table-KiemDinhDienBBTT').html();
        var render = "";
        ...
        $.ajax({
            type: 'GET',
            url: '/admin/KDDienBBTT/ListGDNDienTTXL', //  TTKiemTraKiemDinh = 2 
            data: {
            ...
            success: function (response) {
                ...
                $('#lbKiemDinhDienBBTTTotalRecords').text(response.Result.RowCount);

                if (render !== '') {
                    $('#tblContentKiemDinhDienBBTT').html(render);
                }

                if (response.Result.RowCount !== 0) {
                    wrapPagingKiemDinhDienBBTT(response.Result.RowCount, function () {
    ...
    function wrapPagingKiemDinhDienBBTT(recordCount, callBack, changePageSize) {
        var totalsize = Math.ceil(recordCount / tedu.configs.pageSize);
        //Unbind pagination if it existed or click change pagesize
        if ($('#paginationULKiemDinhDienBBTT a').length === 0 || changePageSize === true) {
            $('#paginationULKiemDinhDienBBTT').empty();
            $('#paginationULKiemDinhDienBBTT').removeData("twbs-pagination");
            $('#paginationULKiemDinhDienBBTT').unbind("page");
        }
        //Bind Pagination Event
        $('#paginationULKiemDinhDienBBTT').twbsPagination({
    ...
    function loadGDNXuLyKiemDinhDien(giaydenghiDMCCDVDienId) {
        $.ajax({
            type: "GET",
            url: "/Admin/KDDienBBTT/GetGDNDMCCDienId",
            data: {
            ...
            success: function (response) {
                ...
                if (ttkiemtrakiemdinh == 2 && ttxulykiemdinh == 3) { // 
                    ...
                    loadGDNXuLyKiemDinhDienUp(giaydenghiDMCCDVDienId);                   
                }
                tedu.stopLoading();
            },
            error: function () {
    ...
    function saveGDNXuLyKDDienBBTT() {
        ...
        if (isMainValidate === true) {            
            ...
            $.ajax({
                type: "POST",
                url: "/Admin/KDDienBBTT/XLKDDienBBTT",
                data: {
    ...
    function updateGDNXuLyKDDienBBTT() {
        var giaydenghidmcungcapnuocId = $('#hidGiayDeNghiDMCungCapDienId').val();
        ...
        if (isMainValidate === true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Admin/KDDienBBTT/XLKDDienBBTTUp",
                data: {
                    GiayDeNghiDMCungCapDienId: giaydenghidmcungcapnuocId,
    ...
    function loadGDNXuLyKiemDinhDienUp(giaydenghiDMCCDVDienId) {
        $.ajax({
            type: "GET",
            url: "/Admin/KDDienBBTT/GetGDNKDDienId",
            data: {
                id: giaydenghiDMCCDVDienId
            },
            dataType: "json",

var addeditgddtController = function () {
    ...
    function loadTableGiayDeNghi(isPageChanged) {
        var template = $('#table-TraCuuGiayDeNghiDien').html();
        var render = "";
        ...
        $.ajax({
            type: 'GET',
            url: '/admin/giaydndt/GetListGDNDien',
            data: {
            ...
            success: function (response) {
                ...
                $('#lbTraCuuGiayDeNghiDienTotalRecords').text(response.Result.RowCount);

                if (render !== '') {
                    $('#tblContentTraCuuGiayDeNghiDien').html(render);
                }
    ...
    function wrapPagingTraCuuGiayDeNghi(recordCount, callBack, changePageSize) {
        ...
        if ($('#paginationULTraCuuGiayDeNghiDien a').length === 0 || changePageSize === true) {
            $('#paginationULTraCuuGiayDeNghiDien').empty();
            $('#paginationULTraCuuGiayDeNghiDien').removeData("twbs-pagination");
            $('#paginationULTraCuuGiayDeNghiDien').unbind("page");
        }
        //Bind Pagination Event
        $('#paginationULTraCuuGiayDeNghiDien').twbsPagination({
    ...
    function XemThongTinXuLy(giaydenghiDMid) {  
        ...
        $.ajax({
            type: 'GET',
            url: '/admin/giaydndt/GDNQTCCN',
            data: {
                GiayDeNghiDMCungCapDienId: giaydenghiDMid                
            },
            dataType: 'json',
            success: function (response) {       
                if (response.Result.Results.length === 0) {

var chuyentknuocController = function () {
    var userName = $("#hidUserName").val();

    this.loadEditXacNhanDkNuoc = function () {
        loadEditXacNhanDkNuoc();
    }
    ...
    function registerEvents() {
        $('#txtNgayChuyenTKGDNNuoc ').datepicker({
            autoclose: true,
            format: 'dd/mm/yyyy',
            language: 'vi'
        });

        $('#btnSaveChuyenTKNuoc').on('click', function () {
    ...
    function clearChuyenTK() {
        ...
        $('#txtNgayChuyenTKGDNNuoc').val(tedu.getFormattedDate(datenow));
        $("#txtNhanVienChuyenTKGDNNuoc").val('');
        //$("#ddlToPhongGDNNuoc")[0].selectedIndex = 0;
        $("#txtGhiChuChuyenTKGDNNuoc").val('');
    }
    ...
    function saveXacNhan() {
        ...
        var ngayxacnhan = tedu.getFormatDateYYMMDD($('#txtNgayChuyenTKGDNNuoc').val());
        //var nhanvientuchoi = $("#txtNhanVienTuChoiGDNDien").val();
        var ghichuxacnhan = $("#txtGhiChuChuyenTKGDNNuoc").val();

        $.ajax({
            type: "POST",
            url: "/Admin/ttdangkytt/XacNhanDkNuoc",
            data: {
            ...
            success: function (response) {
                if (response.Result === false) {
                    tedu.notify("Lưu xác nhận đăng ký nước hồ sơ.", "error");
                }
                else {
                    nguyen.appUserLoginLogger(userName, "Lưu xác nhận giấy đăng ký nước.");

                    tedu.notify('Lưu xác nhận đăng ký nước hồ sơ.', 'success');

                    loadTableTTDangKyNuoc(true);

                    $('#modal-add-edit-ChuyenTKNuoc').modal('hide');
                    tedu.stopLoading();
                }
            },
            error: function () {
                tedu.notify('Có lỗi! Không thể Lưu xác nhận đăng ký nước hồ sơ.', 'error');
                tedu.stopLoading();
            }
        });
    }
    ...
    function loadEditXacNhanDkNuoc() {
        var dangkynuocid = $('#hidTTDangKyNuoc').val();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#txtNgayChuyenTKGDNNuoc').val(tuchoi.NgayChuyenTK == "0001-01-01T00:00:00" ?
                    tedu.getFormattedDate(datenow) : tedu.getFormattedDate(tuchoi.NgayChuyenTK));
                $("#txtNhanVienChuyenTKGDNNuoc").val(tuchoi.TenNhanVienChuyenTK == "null" ? '' : tuchoi.TenNhanVienChuyenTK);
                $("#txtGhiChuChuyenTKGDNNuoc").val(tuchoi.LyDoChuyenTK == "null" ? '' : tuchoi.LyDoChuyenTK);

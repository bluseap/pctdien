var addeditthutuyenController = function () {
    ...
    function registerEvents() {
        ...
        $("#ddl-show-pageHoSoQDTT").on('change', function () {
            tedu.configs.pageSize = $(this).val();
            tedu.configs.pageIndex = 1;
            LoadTableHoSo(true);
        });

        $('body').on('click', '.btn-editHoSoQDTT', function (e) {
            e.preventDefault();
    ...
    function LoadTableHoSo(isPageChanged) {
        var template = $('#table-HoSoQDTT').html();
        var render = "";
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#lblHoSoQDTTTotalRecords').text(response.Result.RowCount);

                if (render !== '') {
                    $('#tblContentHoSoQDTT').html(render);
                }
    ...
    function wrapPagingHoSo(recordCount, callBack, changePageSize) {
        ...
        if ($('#paginationULHoSoQDTT a').length === 0 || changePageSize === true) {
            $('#paginationULHoSoQDTT').empty();
            $('#paginationULHoSoQDTT').removeData("twbs-pagination");
            $('#paginationULHoSoQDTT').unbind("page");
        }
        //Bind Pagination Event
        $('#paginationULHoSoQDTT').twbsPagination({

var addeditpokylaihdnuocController = function () {
    ...
    this.loadTableThayHopDongNuoc = function () {
        loadTableThayHopDongNuoc(true);
    }
    ...
    function registerEvents() {
        ...
        $('#btnSaveEditThayHD').on('click', function () {
            var isthayhopdong = $('#hidInsertUpdateThayHopDongId').val(); // 1: insert; 2: update; 
    ...
    function isFormMainValidate() {
        if ($('#frmMainEditThayHopDongNuoc').valid()) {
            return true;
        }
        else {
            return false;
    ...
    function formMainValidate() {
        ...
        //Init validation
        $('#frmMainEditThayHopDongNuoc').validate({
            errorClass: 'red',
            ignore: [],
            language: 'vi',
            rules: {
    ...
    function loadTableThayHopDongNuoc(isPageChanged) {
        ...
        $.ajax({
            type: 'GET',
            url: '/admin/PoKyLaiHDNuoc/GetListTHD',
            data: {
                KhuVuc: khuvuc,
            ...
            success: function (response) {
                if (response.Result.Results.length === 0) {
                    ...
                    $.each(response.Result.Results, function (i, item) {
                        render += Mustache.render(template, {
                            IDTHD: item.IDTHD,
                            ThayHopDongIdMaKv: item.ThayHopDongIdMaKv,
                            ...
                            TTThayHopDongNuoc: nguyen.getTTThayHopDongNuoc(item.TTThayHopDongNuoc)                                             
                ...
                if (response.Result.RowCount !== 0) {
                    wrapPagingThayHopDongNuoc(response.Result.RowCount, function () {
                        loadTableThayHopDongNuoc();
                    },
                        isPageChanged);
                }
            },
    ...
    function wrapPagingThayHopDongNuoc(recordCount, callBack, changePageSize) {
    ...
    function saveThayHopDong() {
        var thayhopdongid = $('#hidThayHopDongId').val();
        ...
        if (isMainValidate === true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Admin/PoKyLaiHDNuoc/SaveThayHDong",
                data: {
                    IDTHD: thayhopdongid,
                ...
                beforeSend: function () {
                    nguyen.startLoading();
                    $('#modal-add-edit-EditThayHopDongNuoc').modal('hide');
                },
                success: function (response) {
                    if (response.Result === false) {
                        ...
                        nguyen.appUserLoginLogger(userName, 'Nhập thay hợp đồng Nước mới');
    ...
    function loadKhoaSo() {
        ...
        $.ajax({
            type: "POST",
            url: "/Admin/PoKyLaiHDNuoc/KhoaSo",
            data: {      
                MaKhachHang: makhachhang,                
                NAM: namthay,
                THANG: thangthay
    ...
    function loadEditThayDongHo() {
        ...
        $.ajax({
            type: "GET",
            url: "/Admin/PoKyLaiHDNuoc/GetTHDId",
            data: {
    ...
    function updateThayHopDong() {
        var thaydonghoidmakv = $('#hidThayHopDongIdMaKv').val();        
        ...
        if (isMainValidate === true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Admin/PoKyLaiHDNuoc/UpThayHDong",
                data: {
                    ThayHopDongIdMaKv: thaydonghoidmakv,                   
                ...
                success: function (response) {
                    if (response.Result === false) {
                        ...
                        nguyen.appUserLoginLogger(userName, 'Sửa thay hợp đồng Nước mới');

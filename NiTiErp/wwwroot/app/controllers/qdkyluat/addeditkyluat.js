var addeditkyluatController = function () {
    ...
    function registerEvents() {
        ...
        $("#ddl-show-pageHoSoQDKL").on('change', function () {
            tedu.configs.pageSize = $(this).val();
            tedu.configs.pageIndex = 1;
            LoadTableHoSo(true);
        });

        $('body').on('click', '.btn-editHoSoQDKL', function (e) {
            e.preventDefault();
    ...
    function loadLoaiQuyetDinh() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#ddlLoaiQuyetDinh').val("KL03"); //Quyet dinh ky luat
            },
            error: function (status) {
                console.log(status);
    ...
    function loadHinhThucKyLuat() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#ddlLoaiHinhThucKyLuat').html(render);
            },
            error: function (status) {
                console.log(status);
    ...
    function LoadTableHoSo(isPageChanged) {
        var template = $('#table-HoSoQDKL').html();
        var render = "";
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#lblHoSoQDKLTotalRecords').text(response.Result.RowCount);

                if (render !== '') {
                    $('#tblContentHoSoQDKL').html(render);
                }
    ...
    function wrapPagingHoSo(recordCount, callBack, changePageSize) {
        ...
        if ($('#paginationULHoSoQDKL a').length === 0 || changePageSize === true) {
            $('#paginationULHoSoQDKL').empty();
            $('#paginationULHoSoQDKL').removeData("twbs-pagination");
            $('#paginationULHoSoQDKL').unbind("page");
        }
        //Bind Pagination Event
        $('#paginationULHoSoQDKL').twbsPagination({

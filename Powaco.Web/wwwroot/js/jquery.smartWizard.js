...
function isFormMainValidateHinh() {
    if ($('#frmDangKyNuocHinh').valid()) {
        return true;
    }
    else {
        return false;
...
function isFormMainValidateHinhMucDich() {
    if ($('#frmDangKyNuocHinhMucDich').valid()) {
        return true;
    }
    else {
        return false;
...
function formMainValidateHinhMucDich() {
    jQuery.validator.addMethod("isDanhMuc", function (value, element) {
        if (value === "0")
            return false;
        else
    ...
    //Init validation
    $('#frmDangKyNuocHinhMucDich').validate({
        errorClass: 'red',
        ignore: [],
        language: 'vi',
        rules: {            
            ddlGiayToXacDinhMucDich: {
                required: true,
                isDanhMuc: true
            },
        },
...
function SmartWizard(target, options) {
    ...
    var _init = function($this) {
        ...
        $($this.buttons.next).click(function () {
            ...
            let isMainValidateHinh = isFormMainValidateHinh();
            let isMainValidateHinhMucDich = isFormMainValidateHinhMucDich();

            let mucdichsudung = $('#ddlMucDichSuDung').val();  
            
            //Kiem tra bang TTDMDangKy theo Muc dich su dung
            if (mucdichsudung === '1') {
                if (isMainValidate === true && isMainValidateHinh === false) {
                    bienStep = bienStep + 1;
                    $this.goForward();
                }
                else if (isMainValidate === true && isMainValidateHinh === true) {
                    bienStep = bienStep + 1;
                    $this.goForward();
                }
            }
            //Kiem tra bang TTDMDangKy theo Muc dich su dung
            else if (mucdichsudung === '2') {
                if (isMainValidate === true && isMainValidateHinh === false && isMainValidateHinhMucDich === false) {
                    bienStep = bienStep + 1;
                    $this.goForward();
                }
                else if (isMainValidate === true && isMainValidateHinh === true && isMainValidateHinhMucDich === false) {
                    if (bienStep == 0) {
                        bienStep = bienStep + 1;
                        $this.goForward();
                    }
                    else if (bienStep == 1) {
                        if (isMainValidate === true && isMainValidateHinh === true && isMainValidateHinhMucDich === true) {
                            bienStep = bienStep + 1;
                            $this.goForward();
                        }
                    }
                }
                else if (isMainValidate === true && isMainValidateHinh === true && isMainValidateHinhMucDich === true) {
                    bienStep = bienStep + 1;
                    $this.goForward();
                }
            }            
...
(function ($) {
    ...
    formMainValidate();
    formMainValidateHinh();
    formMainValidateHinhMucDich();
...
$.fn.smartWizard.defaults = {
    ...
    onFinish: () => {  // triggers when Finish button is clicked
        ...
        var mucdichsudung = $('#ddlMucDichSuDung').val();
        var laoihinhdichvu = $('#ddlLoaiHinhDichVu').val();
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.Results.length === 0) {
                    ...
                    powa.notify('Đăng ký lắp mới.', 'success');
                    ...
                    $('#hidTTDangKyNuocId').val(response.RowCount);

                    loadDataDangKyNuocId();
                }
            },
            //cache: false,
            error: function (err) {
...
function loadDataDangKyNuocId() {
    var dangkuynuocid = $('#hidTTDangKyNuocId').val();

    $.ajax({
        type: 'GET',
        url: '/dangkynuoc/GetId',
        data: {
            dangkynuocId: dangkuynuocid
        },
        dataType: "json",
        success: function (response) {
